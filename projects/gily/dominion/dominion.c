...
int callAdventurer(struct gameState *state) {
	int currentPlayer = whoseTurn(state);
	int drawntreasure=0;
...
int callVillage(struct gameState *state, int handPos) {
	int currentPlayer = whoseTurn(state);
	...
	if (choice1 > 0){//Boolean true or going to discard an estate
		...
		while(card_not_discarded){
			if (state->hand[currentPlayer][p] == estate){//Found an estate card!
			    state->coins += 8;//Add 4 coins to the amount of coins
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
      return callMine(state, choice1, choice2, handPos);
    ...
    case smithy:
      //+3 Cards
      return callSmithy(state, handPos);
		
    case village:
      return callVillage(state, handPos);
		
    case baron:
      return callBaron(state, choice1);
